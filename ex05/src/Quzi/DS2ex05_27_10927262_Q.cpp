...
public:
    ...
    void SaveToAdjFile(const std::string& file_number) {
        ...
        buffer << "<<< There are " << node_count_ << " nodes in adjacency lists. >>>\n";
    ...
    void SaveToCCFile(const std::string& file_number) {
        ...
        sorted_indices_.resize(component_sets_.size());
        ...
        std::string msg = "<<< There are " + std::to_string(sorted_indices_.size()) 
        ...
        for (const size_t& index : sorted_indices_) {
            ...
            for (size_t node_id : component_sets_[index]) {
                ...
                if ((node_counter & 7) == 0 || node_counter == set_size) {  
                    buffer_file << '\n';
                } else {
                    buffer_file << "\t";
                }
    ...
    void SaveToDsFile(const std::string& file_number, const NodeType& source) {
        ...
        for (const auto& result : path_min_value_) {
            ...
            if ((result_count & 7) == 0 || result_count == path_min_value_.size()) {
                buffer << "\t\n";
            } else {
                buffer << "\t";
            }
    ...
    */
    ...
    /**
     * @brief Computes the Minimum Spanning Tree (MST) weight for a connected component using Prim's algorithm.
     * 
     * @param start_node The starting node for Prim's algorithm
     * @param cid The component ID specifying which connected component to process
     * @return float The total weight of the MST for this component
     */
    void PrintAllMSTs() {
        std::ostringstream buffer;
        size_t counter = 0;

        for (const size_t& cid : sorted_indices_) {
            if (component_sets_[cid].empty()) continue;

            size_t start_node = component_sets_[cid][0];
            float mst_cost = ComputeMST(start_node, cid);
            buffer << std::fixed << std::setprecision(2);
            buffer << "The MST cost of connected component {"  << std::right << std::setw(2)
                    << ++counter << "} = " << mst_cost << "\n";
        }
 ...
 private:
    ...
    */
    ...
    float ComputeMST(const size_t& start_node, const size_t& cid) {
        // use prime algo.
        // Min-heap priority queue: <distance, node_id>
        using MinHeap = std::priority_queue<std::pair<float, size_t>,
                                            std::vector<std::pair<float, size_t>>,
        ...
        const std::vector<size_t>& component_nodes = component_sets_[cid];
        std::unordered_set<size_t> visited(component_nodes.begin(), component_nodes.end());
        ...
        while (!min_heap.empty()) {
            ...
            for (const Edge& edge : adj_list_[node]) {
                if (visited.find(edge.neighbor_id) != visited.end()) {
                    min_heap.push({edge.weight, edge.neighbor_id});
                }
            }
        }
...
static void Task3(UnDirectedGraph<std::string>& undir_graph) {
    undir_graph.PrintAllMSTs();
}  // Task3()
