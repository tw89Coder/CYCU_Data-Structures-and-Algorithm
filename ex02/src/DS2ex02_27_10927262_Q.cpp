...
class BTree {
 ...
 private:
    ...
    int getMaxKeys() const {
        ...
        return order_ - 1;  // For a standard B-tree
    }
    ...
    */
    ...
    void inorderTraversalHelper(const BTreeNode<T>* node,
        ...
        if (!node->children.empty()) {
            inorderTraversalHelper(node->children.back(), result, keyword);
        }
    }
};
...
void Task3(const std::vector<UniversityDepartment> &graduate_info_list,
    ...
    for (int order : department_search) {
        if (set.find(order) != set.end()) {
            intersection.push_back(order);
        }
    }
...
int main() {
    ...
    do {
        ...
        case 2:
            ...
            break;
        case 3:
            if (graduate_info_b_tree_order_3.isEmpty()) {
                std::cout << "### Choose 1 first. ###\n";
            } else if (graduate_info_avl.isEmpty()) {
                std::cout << "### Choose 2 first. ###\n";
            } else {
                Task3(graduate_info_list, graduate_info_b_tree_order_3, graduate_info_avl);
            }
