...
class DirectedGraph {
 public:
    ...
    void AddEdge(const NodeType& publisher, const NodeType& subscriber, float weight) {
        ...
        if (adj_list[publisher].size() == 1) {
            ...
            reachable_vec[publisher] = {};  // Avoid rehashing
            weight_reachable_vec[publisher] = {};
        }
    ...
    void SaveToInfFile(const std::string& file_number) const {
        ...
        buffer << "<<< There are " << total_node_count << " IDs in total. >>>\n";

        // Process each publisher node
        for (size_t i = 0; i < weight_publisher_list.size(); ++i) {
            const NodeType& key = weight_publisher_list[i].first;

            auto graph_node = weight_reachable_vec.find(key);
            if (graph_node != weight_reachable_vec.end()) {
                std::vector<std::pair<NodeType, float>>& nodes = graph_node->second;
                ...
                // Format publisher header with connection count
                buffer << "[" << std::setw(3) << i + 1 << "] " << key
                       << "(" << weight_publisher_list[i].second << "): \n";
                ...
                // Format nodes in path
                for (size_t j = 0; j < nodes.size(); ++j) {
                    buffer << "\t(" << std::setw(2) << j + 1 << ") " << nodes[j].first;
    ...
    */
    void ComputeAllConnectionWtCounts(const std::string& mode,
        ...
        weight_reachable_vec.reserve(64 * 1024);
    ...
    void Perf() {
        ...
        weight_reachable_vec.max_load_factor(0.25);
        weight_reachable_vec.reserve(64 * 1024);
    ...
    size_t GetnTotalNodeCount() const {
 ...
 private:
    ...
    void RunSimpleBFS(const NodeType& source_node) {
        ...
        while (!queue.empty()) {
...

            {
                ...
                const auto& cache = reachable_vec.find(current);
                if (cache != reachable_vec.end() && !cache->second.empty()) {
    ...
    */
    ...
    mutable std::unordered_map<NodeType, std::vector<std::pair<NodeType, float>>> weight_reachable_vec;
    mutable std::vector<std::pair<NodeType, size_t>> publisher_list;
    mutable std::vector<std::pair<NodeType, size_t>> weight_publisher_list;
    ...
    do {
        ...
        if (isFloat(input_float)) {
            ...
            if (0.0f <= value && value <= 1.0f) {
                ...
                std::cout << '\n';
                std::cout << "### " << input_float << " is NOT in [0,1] ###\n";
            }
        }
